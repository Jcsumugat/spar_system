...
export default function Index({ auth, businesses, barangays, filters }) {
    ...
    const [permitHistoryModal, setPermitHistoryModal] = useState(false);
    ...
    const getPermitStatusBadge = (status) => {
        const statusConfig = {
            ...
            "Expiring Soon": { bg: "bg-yellow-100", text: "text-yellow-800" },
        ...
        return (
            <span
                className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${config.bg} ${config.text}`}
            >
                {status}
            </span>
        );
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            ...
            {/* Permit History Modal */}
            {permitHistoryModal && selectedBusiness && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                            ...
                            <button
                                onClick={() => setPermitHistoryModal(false)}
                                className="text-gray-400 hover:text-gray-600"
                            >
                                <X className="w-6 h-6" />
                            </button>
                        ...
                        <div className="p-6">
                            {selectedBusiness.sanitary_permits &&
                            selectedBusiness.sanitary_permits.length > 0 ? (
                                <div className="space-y-4">
                                    {selectedBusiness.sanitary_permits.map(
                                        (permit) => (
                                            <div
                                                key={permit.id}
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-gray-600">
                                        This business has no sanitary permits on
                                        record.
                                    </p>
                                </div>
                            )}
